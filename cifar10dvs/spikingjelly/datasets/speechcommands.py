...
class SPEECHCOMMANDS(Dataset):
    def __init__(self,
        ...
        # print(basename, '_basename')

        archive = os.path.join(root, basename)
        # print(archive, '_archive')
        basename = basename.rsplit(".", 2)[0]
        # print(basename, '_basename2')
        folder_in_archive = os.path.join(folder_in_archive, basename)
        # print(folder_in_archive, '_folder_in_archive')
        self._path = os.path.join(root, folder_in_archive)
        # print(self._path, '_self._path')
        self.noise_list = sorted(str(p) for p in Path(self._path).glob('_background_noise_/*.wav'))
        # print(self.noise_list, '_self.noise_list')
        if download:
