...
from spikingjelly.datasets.cifar10_dvs import CIFAR10DVS as DVS10
...
from models import spiking_resnet, vgg_model, spiking_vgg_bn, sew_resnet
from modules import neuron
from modules import surrogate as surrogate_self
from utils import Bar, AverageMeter, accuracy, static_cifar_util, augmentation, myTransform
from utils.augmentation import ToPILImage, Resize, ToTensor
from utils.cifar10_dvs import CIFAR10DVS, DVSCifar10, CIFAR10_DVS_Aug
from utils.data_loaders import TinyImageNet

from utils.util import DatasetWarpper, DVStransform
from utils.augument import SNNAugmentWide, DVSAugment
...
def main():
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        transform_train = DVStransform(transform=transforms.Compose([
            transforms.RandomCrop(128, padding=12),
            # transforms.Resize(size=64, antialias=True),
            # transforms.RandomHorizontalFlip(p=0.5),
            SNNAugmentWide(num_magnitude_bins=60)
        ])
        )
    ...
    elif args.model in ['SEWResNet', 'sew_resnet18', 'sew_resnet34']:
        net = sew_resnet.__dict__[args.model](neuron=neuron_model, num_classes=num_classes,
                                                  neuron_dropout=args.drop_rate,
                                                  tau=args.tau, surrogate_function=surrogate_function, c_in=c_in,
    ...
    # 新增：打印模型结构，帮助确定神经元所在层
    print("=" * 50)
    print("模型结构如下（用于确定神经元层位置）：")
    print(net)
    print("=" * 50)
    ...
    if args.lr_scheduler == 'StepLR':
        ...
        if args.dataset == 'dvsgesture':
            ...
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.T_max, eta_min=0)
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # # 新增：膜电位记录配置
        # record_membrane = True  # 是否开启记录
        # target_layer = net.layer2[2]  # 目标神经元层（根据实际模型修改）
        # record_epoch_interval = 9  # 每10个epoch记录一次
        # save_dir = os.path.join(out_dir, 'membrane_records')  # 保存目录
        # # print('膜电位记录保存位置', save_dir)
        # os.makedirs(save_dir, exist_ok=True)
        # record_batch = 1  # 只记录第0个batch的数据（避免文件过大）
        ...
        with torch.no_grad():
            for data in test_data_loader:
                ...
                # membrane_potentials = []
                # # 标记当前batch是否需要记录
                # need_record = record_membrane and (epoch % record_epoch_interval == 0) and (batch_idx == record_batch)
                ...
                for t in range(t_step):
                    ...
                    # # 新增：记录当前时间步的膜电位
                    # # print(target_layer.u)
                    # if need_record:
                    #     # 获取膜电位并转为numpy（只保留第一个样本，避免数据量过大）
                    #     if isinstance(target_layer.u, float):
                    #         v_t = torch.zeros_like(target_layer.v[:, 0, :, :]).detach().cpu().numpy()
                    #     else:
                    #         v_t = target_layer.u[:, 0, :, :].detach().cpu().numpy()  # [0]表示取第一个样本
                    #     # print(target_layer.u[:, 0, :, :].size())
                    #     membrane_potentials.append(v_t)
                ...
                # # 新增：保存膜电位数据（当前batch处理完毕后）
                # if need_record:
                #     # 保存为numpy文件（形状：[时间步, 通道, 高度, 宽度]）
                #     save_path = os.path.join(save_dir, f'membrane_epoch_{epoch}_batch_{batch_idx}.npy')
                #     np.save(save_path, np.array(membrane_potentials))
                #     print(f"已保存膜电位数据至: {save_path}")
                #     # 关闭当前epoch的记录开关（避免重复记录）
