...
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, dropout, neuron: callable = None, **kwargs):
        ...
        self.relu1 = neuron(**kwargs, channel=in_channels)
        self.relu2 = neuron(**kwargs, channel=out_channels)
...
class PreActResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes, dropout, neuron: callable = None, **kwargs):
        ...
        self.relu1 = neuron(**kwargs, channel=512)
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        # print("layer4输出维度:", out.shape)  # 查看layer4输出维度
        out = self.relu1(self.bn1(out))
        out = self.pool(out)
        # print("池化后维度:", out.shape)  # 查看池化后维度
        out = self.flat(out)
        # print("展平后维度:", out.shape)  # 查看展平后维度
        out = self.drop(out)
        out = self.linear(out)
        return out
